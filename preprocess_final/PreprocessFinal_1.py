file_name_ouput = 'PreprocessFinal_1'
    ...
    for line in f: 
        ...
        if lnum == 1:
           ...
           if userid is not 'unknown':
               #print ReviewID
               fout.write("%d,%s,%s,%s,%s,%s,%s,%d,%s,%s,%s\n" %(ReviewID,productid,title,price,userid,profilename,helfpulness,int(str(score).strip('.0')),time,summary,reviewtext))     
               ReviewID = ReviewID+1
        linenumber += 1
fout.close()
